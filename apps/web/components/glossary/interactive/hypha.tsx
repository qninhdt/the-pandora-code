    ...
    >
      <div className="absolute inset-0">
        <svg viewBox="0 0 100 100" className="h-full w-full" preserveAspectRatio="xMidYMid meet" role="img" aria-label={t("title")}>
          ...
          {/* nutrient patch */}
          <circle cx={nutrient.x} cy={nutrient.y} r={6 + growth * 2} fill="var(--amber)" opacity={0.35} />
          <circle cx={nutrient.x} cy={nutrient.y} r="2.5" fill="var(--amber)" opacity={0.85} />
          <text x={nutrient.x} y={nutrient.y - 9} textAnchor="middle" style={{ fontSize: 2.2, fontFamily: "monospace", fill: "var(--amber)" }}>
            {t("nutrient")}
          </text>
          {/* hypha */}
          <path d={d} fill="none" stroke="var(--magenta)" strokeWidth="1.4" strokeLinecap="round" opacity={0.9} />
          {branches.map((b, i) => (
            <line key={i} x1={b.x1} y1={b.y1} x2={b.x2} y2={b.y2} stroke="var(--teal)" strokeWidth="0.9" opacity={0.75} />
          ))}
          <circle cx={tip.x} cy={tip.y} r={2 + growth} fill="var(--cyan)" style={{ filter: "drop-shadow(0 0 4px var(--cyan))" }} />
        </svg>
        ...
        <div className="absolute inset-x-3 bottom-10 space-y-2">
          <ControlSlider label={t("growth")} value={growth} min={0.1} max={1} step={0.01} display={`${Math.round(growth * 12)} µm/h`} onChange={setGrowth} thumb="magenta" />
          <ControlSlider label={t("steer")} value={steer} min={0} max={1} step={0.01} display={t("nutrient")} onChange={setSteer} thumb="amber" />
        </div>
      </div>
    </GlossaryFrame>
  );
