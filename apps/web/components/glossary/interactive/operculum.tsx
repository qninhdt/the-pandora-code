...
export default function Operculum() {
  ...
  return (
    <GlossaryFrame
      ...
      caption={
        <span className={ram ? "text-cyan" : "text-teal"}>{ram ? t("ram") : t("pump")}</span>
      }
    >
      <div className="absolute inset-0">
        ...
        <div className="absolute inset-x-3 bottom-12 flex justify-center gap-1.5">
          <button
            type="button"
            onClick={() => setRam(false)}
            className="rounded-lg border px-3 py-1 font-mono text-[10px] uppercase"
            style={{
              borderColor: !ram ? "var(--teal)" : "var(--border-strong)",
              color: !ram ? "var(--teal)" : "var(--muted)",
              background: "var(--void)",
            }}
          >
            {t("pump")}
          ...
          <button
            ...
            className="rounded-lg border px-3 py-1 font-mono text-[10px] uppercase"
            style={{
              borderColor: ram ? "var(--cyan)" : "var(--border-strong)",
              color: ram ? "var(--cyan)" : "var(--muted)",
              background: "var(--void)",
            }}
          >
            {t("ram")}
