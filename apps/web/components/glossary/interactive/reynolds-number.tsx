    ...
    >
      <div className="absolute inset-0">
        <svg
          ...
          aria-label={t("title")}
        >
          <circle cx="30" cy="42" r={6 + size * 10} fill="var(--surface)" stroke="var(--cyan)" strokeWidth="0.8" />
          {Array.from({ length: turbulent ? 9 : 5 }, (_, i) => {
            const y = 24 + i * 6;
            if (!turbulent) {
              return (
                <line key={i} x1="45" y1={y} x2="88" y2={y} stroke="var(--teal)" strokeWidth="0.5" opacity="0.55" />
              );
            }
            return (
              <path
        ...
        <div className="absolute right-3 top-14 flex flex-col gap-1">
          <Readout label={t("re")} value={re.toExponential(1)} accent={turbulent ? "amber" : "cyan"} />
        </div>
        ...
        <div className="absolute inset-x-3 bottom-10 flex flex-col gap-1.5">
          ...
          <ControlSlider label={t("size")} value={size} min={0.05} max={2} step={0.05} display={`${size.toFixed(2)} m`} onChange={setSize} thumb="cyan" />
          <ControlSlider label={t("speed")} value={speed} min={0.5} max={30} step={0.5} display={`${speed.toFixed(1)} m/s`} onChange={setSpeed} thumb="teal" />
        </div>
      </div>
    </GlossaryFrame>
  );
