    ...
    >
      <div className="absolute inset-0">
        <svg viewBox="0 0 100 100" className="h-full w-full" preserveAspectRatio="xMidYMid meet" role="img" aria-label={t("title")}>
          {on.reflector && <path d="M30 60 Q 50 20 70 60" fill="none" stroke="var(--amber)" strokeWidth="1.2" />}
          {on.photocyte && (
            <circle cx="50" cy="52" r="8" fill="var(--teal)" opacity="0.8" style={{ filter: "drop-shadow(0 0 8px var(--teal))" }} />
          )}
          {on.lens && <ellipse cx="50" cy="40" rx="10" ry="4" fill="var(--cyan)" opacity="0.35" />}
          {on.pigment && <rect x="38" y="34" width="24" height="6" fill="var(--void)" stroke="var(--magenta)" strokeWidth="0.6" />}
          {beam > 0 && (
            <path d={`M50 34 L${50 - 10 * beam} 12 L${50 + 10 * beam} 12 Z`} fill="var(--cyan)" opacity={0.25 * beam} />
          )}
        </svg>
        ...
        <div className="absolute inset-x-2 bottom-11 flex flex-wrap justify-center gap-1">
          {LAYERS.map((k) => (
            <button key={k} type="button" onClick={() => setOn((s) => ({ ...s, [k]: !s[k] }))}
              className="rounded-lg border px-2 py-1 font-mono text-[9px] uppercase"
              style={{ borderColor: on[k] ? "var(--cyan)" : "var(--border-strong)", color: on[k] ? "var(--cyan)" : "var(--muted)", background: "var(--void)" }}>
              {t(k)}
            </button>
          ))}
        </div>
