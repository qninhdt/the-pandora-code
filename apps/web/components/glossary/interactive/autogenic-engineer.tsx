...
export default function AutogenicEngineer() {
  ...
  const tone = stage === "old" ? "var(--cyan)" : stage === "mature" ? "var(--teal)" : "var(--muted)";
    ...
    >
      <div className="absolute inset-0">
        <svg viewBox="0 0 100 78" className="h-full w-full" preserveAspectRatio="xMidYMid meet" role="img" aria-label={t("title")}>
          <line x1="10" y1="56" x2="90" y2="56" stroke="var(--border-strong)" strokeWidth="0.5" opacity={0.6} />
          <ellipse cx="50" cy="56" rx={spread * 0.8} ry="2.4" fill={tone} opacity={0.18} />
          <line x1="50" y1="56" x2="50" y2={56 - h} stroke={tone} strokeWidth={1 + size * 2.4} strokeLinecap="round" opacity={0.9}
            style={{ filter: `drop-shadow(0 0 ${2 + size * 4}px ${tone})` }} />
          {/* crown widens with age, so the habitat it constitutes widens too */}
          <path d={`M${50 - spread} ${56 - h + 6} Q50 ${56 - h - 6} ${50 + spread} ${56 - h + 6}`} fill="none" stroke={tone} strokeWidth={0.8 + size} opacity={0.7} />
          {spots.slice(0, tenants).map(([fx, fy], i) => (
            <circle key={i} cx={50 + fx * spread} cy={56 - h * (0.3 + fy * 0.6)} r="1.6" fill="var(--amber)" opacity={0.85}
        ...
        <div className="absolute right-3 top-14">
          <Readout label={t("tenants")} value={tenants} accent={stage === "old" ? "cyan" : stage === "mature" ? "teal" : "cyan"} />
        </div>
