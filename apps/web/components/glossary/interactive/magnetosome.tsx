...
export default function Magnetosome() {
  ...
  return (
    <GlossaryFrame
      ...
      caption={
        <span className={on ? "text-cyan" : "text-muted"}>{on ? t("fieldOn") : t("fieldOff")}</span>
      }
    >
      <div className="absolute inset-0">
        ...
        >
          ...
          {[-15, -9, -3, 3, 9, 15].map((d) => (
            <circle
              key={d}
              cx={50 + d * Math.cos((angle * Math.PI) / 180)}
              cy={50 + d * Math.sin((angle * Math.PI) / 180)}
              r="2.4"
              fill="var(--amber)"
            />
          ))}
        ...
        <div className="absolute inset-x-3 bottom-12 flex justify-center gap-1.5">
          ...
          <button
            ...
            className="rounded-lg border px-3 py-1 font-mono text-[10px] uppercase"
            style={{
              borderColor: !on ? "var(--muted)" : "var(--border-strong)",
              color: !on ? "var(--muted)" : "var(--muted)",
              background: "var(--void)",
            }}
          >
            {t("fieldOff")}
