...
export default function UnidirectionalAirflow() {
  ...
  return (
    <GlossaryFrame
      ...
      caption={
        <span className={uni ? "text-teal" : "text-amber"}>{uni ? t("uni") : t("tidal")}</span>
      }
    >
      <div className="absolute inset-0">
        ...
        >
          ...
          {uni
            ...
            : Array.from({ length: 5 }, (_, i) => (
                <path
                  key={i}
                  d={`M24 ${40 + i * 3} h52 M76 ${40 + i * 3} h-52`}
                  stroke="var(--amber)"
                  strokeWidth="0.6"
                  opacity="0.55"
                />
        ...
        <div className="absolute inset-x-3 bottom-12 flex justify-center gap-1.5">
          <button
            ...
            style={{
              borderColor: uni ? "var(--teal)" : "var(--border-strong)",
              color: uni ? "var(--teal)" : "var(--muted)",
              background: "var(--void)",
            }}
          >
            {t("uni")}
          ...
          <button
            type="button"
            onClick={() => setUni(false)}
            className="rounded-lg border px-3 py-1 font-mono text-[10px] uppercase"
            style={{
              borderColor: !uni ? "var(--amber)" : "var(--border-strong)",
              color: !uni ? "var(--amber)" : "var(--muted)",
              background: "var(--void)",
            }}
          >
            {t("tidal")}
