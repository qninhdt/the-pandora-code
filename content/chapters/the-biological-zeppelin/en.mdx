...
<DiagramFigure
  ...
  labels={[
    { x: 50, y: 16, side: "right", label: "Gas envelope dome", note: "Translucent bell holding metabolically generated hydrogen" },
    { x: 50, y: 28, side: "right", label: "Marginal sensory belt", note: "Equatorial ring of photoreceptors watching the ground below" },
    { x: 30, y: 54, side: "left", label: "Suspended gondola", note: "Woven basket platform for Na'vi navigators and cargo" },
    { x: 30, y: 80, side: "right", label: "Trailing tentacles", note: "Electrogenic curtain swept through the air to stun and seize prey" }
  ]}
  figNo="01"
...
Every flier we have examined has been a machine for solving one equation: make enough <GlossaryTerm slug="lift">lift</GlossaryTerm> to cancel your weight, and pay for it in muscle, every second, forever. Stop paying and you come down. That is what flight *is*, on Earth and on Pandora, for every bird and bat and banshee and for the great toruk itself.
...
That distinction sounds pedantic and is not. It is the difference between understanding <GlossaryTerm slug="aerostat">aerostats</GlossaryTerm> and repeating a very common piece of nonsense about them. The nonsense goes: hydrogen has half the molar mass of helium, so a hydrogen balloon lifts twice as much. Two grams per mole against four — surely double.
...
<DiagramFigure
  ...
  labels={[
    ...
    { x: 75, y: 52, side: "right", label: "Distributed load webbing", note: "Catenary bridle spreads the load as line tension along the envelope" },
...
<DiagramFigure
  ...
  labels={[
    ...
    { x: 65, y: 45, side: "left", label: "Pandoran atmospheric aerostat", note: "Gigantic gas bag floating on the density difference with dense Pandoran air" },
  ...
  figNo="04"
  caption="The same solution, a hundred and seventy times apart. Physalia builds a gas float, secretes its own lifting gas and vents it to submerge — every component of the Medusoid's buoyancy, already evolved on Earth at thirty centimetres. What Pandora adds is not a new mechanism but two more orders of magnitude."
  tier="real_science"
/>
...
<DiagramFigure
  ...
  labels={[
    { x: 43, y: 78, side: "right", label: "Anaerobic fermentation vat", note: "Microbial symbionts ferment ingested prey to yield H₂" },
...
<DiagramFigure
  ...
  labels={[
    ...
    { x: 52, y: 20, side: "left", label: "Translucent gas dome", note: "Lit from behind rather than from within — the lifting gas is unchanged by the climb" },
    { x: 32, y: 53, side: "left", label: "Peripheral gondolas", note: "Crew tanks the caravan can dump for an immediate climb" }
  ]}
  figNo="06"
...
<DiagramFigure
  ...
  labels={[
    { x: 20, y: 72, side: "right", label: "Lead windray", note: "Supplies horizontal traction and heading, no lift at all" },
    { x: 77, y: 36, side: "left", label: "Towed aerostat", note: "Provides the entire lift, and forty-six tonnes of entrained air to turn" },
    { x: 48, y: 52, side: "right", label: "Tensioned towlines", note: "Braided plant fibre carrying the tugs' pull into the harness" },
    { x: 16, y: 28, side: "right", label: "Crosswind arc", note: "Flying across the wind rather than straight ahead multiplies each tug's pull" }
  ]}
  figNo="07"
...
Now the mitigation, and it is genuine. <GlossaryTerm slug="flammability-limits">Flammability limits</GlossaryTerm> are not fixed properties of a fuel; they depend on what else is in the air. Pandora's atmosphere carries something like seventeen percent carbon dioxide — a figure that has been an antagonist throughout this book, the thing that makes the air unbreathable and drives the exo-pack's whole design. Here, for once, it is on the Na'vi's side. Carbon dioxide is a triatomic molecule with more ways to store energy than nitrogen has, so it absorbs the heat a flame front needs to propagate into the next layer of gas. Add enough and the flammable window narrows from the rich end and eventually closes altogether.
...
<DiagramFigure
  ...
  labels={[
    { x: 56, y: 12, side: "right", label: "Buoyant flame plume", note: "Hydrogen fire climbs away from the envelope rather than falling onto it" },
    { x: 52, y: 75, side: "left", label: "Hearth fire on gondola", note: "Open cooking fire kept well below the gas bag" },
    { x: 52, y: 42, side: "right", label: "Flammable gas envelope", note: "Hydrogen ignites across a wide concentration band and on almost no spark" },
    { x: 10, y: 53, side: "right", label: "Escort windrays", note: "Tugs holding heading while the envelope burns overhead" }
  ]}
  figNo="08"
