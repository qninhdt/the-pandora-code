<DiagramFigure
  ...
  labels={[
    { x: 12, y: 22, side: "right", label: "Field Sampling Extraction", note: "Grace peeling back the forest humus layer to expose root-mycelial junctions." },
    { x: 50, y: 38, side: "right", label: "Primary Taproot Ingress", note: "Lignified root collar branching into high-conductivity conductive filaments." },
    { x: 50, y: 68, side: "left", label: "Electrochemical Synaptic Web", note: "Dense mycorrhizal grid transmitting voltage spikes analogous to neural synapses." },
    { x: 88, y: 60, side: "left", label: "Inter-Tree Mycelial Bridge", note: "Fungal hyphae establishing physical continuity between separate tree root systems." }
  ]}
  figNo="01"
...
<DiagramFigure
  ...
  labels={[
    { x: 21, y: 24, side: "right", label: "Tree of Souls (Primary Hub)", note: "Highest-bandwidth planetary hub with dense synaptic tendrils capable of complete consciousness transfer." },
    { x: 82, y: 26, side: "left", label: "Hometree (Structural Pillar)", note: "Colossal ecological dwelling node, yet non-critical to network topology—its loss did not crash Eywa." },
    { x: 28, y: 70, side: "right", label: "Subsea Spirit Tree (Marine Hub)", note: "Underwater reef node connecting oceanic Cetacean/Metkayina memories into the moon-wide grid." },
    { x: 72, y: 72, side: "left", label: "Whispering Greatcap (Fungal Backbone)", note: "Massive macro-fungal fruiting body demonstrating that fungi, not trees alone, anchor key network switches." }
  ]}
  figNo="05"
...
<DiagramFigure
  ...
  alt="Two large trees stand far apart with a rocky stream between them; below ground their thick root systems reach toward each other but stop across a wide dark gap, while a much finer luminous fungal web crosses beneath the barrier and joins both root systems"
  labels={[
    { x: 18, y: 32, side: "right", label: "Host Tree Root System A", note: "Lignified root systems terminate within meters of the trunk, incapable of direct graft." },
    { x: 50, y: 66, side: "left", label: "Mycelial Transmission Bridge", note: "Sub-millimeter fungal hyphae span the subterranean gap, providing the actual conductive substrate." },
    { x: 82, y: 32, side: "left", label: "Host Tree Root System B", note: "Physically isolated node receiving systemic signals and resource transfers via the fungal intermediary." },
    { x: 50, y: 40, side: "right", label: "Geological Separation Gap", note: "Surface erosion and stream trenches that would sever root contacts are bypassed by mycelial networks." }
  ]}
  figNo="02"
...
<DiagramFigure
  ...
  labels={[
    { x: 28, y: 46, side: "left", label: "Arbuscular Endomycorrhiza", note: "Hyphae penetrate plant cell walls to form microscopic, highly branched arbuscules for direct metabolic exchange." },
    { x: 27, y: 72, side: "left", label: "Cortical Parenchyma Host Cells", note: "Host plant root cells accommodating fungal invaginations without triggering pathogen defense responses." },
    { x: 73, y: 46, side: "right", label: "Ectomycorrhizal Mantle Sheath", note: "Dense fungal mycelium wraps the root exterior, creating a protective barrier and water buffer." },
    { x: 68, y: 28, side: "right", label: "Intercellular Hartig Net", note: "Hyphal network penetrating between epidermal and cortical cells without breaching cell walls." }
  ]}
  figNo="03"
...
<DiagramFigure
  ...
  labels={[
    { x: 22, y: 28, side: "right", label: "Sunlit Mother Tree (Source)", note: "Mature emergent canopy captures abundant solar photons, generating substantial surplus photosynthate." },
    { x: 50, y: 78, side: "left", label: "Mycelial Carbon Pipeline", note: "Luminous hyphal network channels carbon, water, and signaling compounds along source-sink gradients." },
    { x: 72, y: 40, side: "left", label: "Shaded Understory Sapling (Sink)", note: "Light-starved young tree receives vital carbon subsidies and pathogen defense cues from the established network." }
  ]}
  figNo="04"
...
<IsotopeTracerAudit
  caption="What the design buys you. Switch on two distinguishable labels and you can name the donor; add the mesh that cuts hyphae but still passes soil water and you can finally say the carbon went through the fungus rather than leaking into the soil; shade the receiver and you can speak of flow following need — which is exactly, and only, what Simard's experiment earned. The kin claim needs two more controls that field ecology has essentially never managed to run. Nothing here is fraud. It is the difference between a sentence a design supports and a sentence it does not."
/>
...
<DiagramFigure
  ...
  labels={[
    { x: 39, y: 38, side: "left", label: "Neuronal Synaptic Discharge (~1 ms)", note: "Depolarization and neurotransmitter release occur in milliseconds, enabling rapid cognition and reflexes." },
    { x: 71, y: 74, side: "right", label: "Glacial Fungal Calcium Wave (~0.5 cm/min)", note: "Hyphal electrical spikes propagate at centimeters per minute—too slow for real-time planet-wide tactical thought." },
    { x: 76, y: 52, side: "left", label: "Propagation Latency Dial", note: "Signal latency scales with geographic distance, limiting biological networks to slow homeostatic rhythms." }
  ]}
  figNo="06"
...
<DiagramFigure
  ...
  labels={[
    { x: 8, y: 32, side: "right", label: "Empirical Baseline (Two Seedlings)", note: "Controlled lab experiment testing isotopic carbon transfer between two isolated seedlings via one fungal hypha." },
    { x: 43, y: 54, side: "right", label: "Citation Inflation Chain", note: "Secondary and tertiary literature progressively drops experimental caveats, enlarging local transfer into systemic altruism." },
    { x: 82, y: 50, side: "left", label: "Forest-Wide Superorganism Myth", note: "Popular synthesis transforms a resource-trading fungal market into an intentional, utopian planetary internet." }
  ]}
  alt="A small growth chamber with two seedlings joined by one fungal strand sits at the start of a chain of open research pages; across successive copied images the single link swells into more trees and a brighter web, ending as an unsupported vision of an entire forest deliberately feeding seedlings"
  figNo="07"
  caption="How a narrow result grows in the retelling. At the source: two plants, one shared fungal link, a small measured transfer. With each citation the caveat is copied less faithfully and the picture grows — from a possible route, to a forest-wide web, to mature trees deliberately nursing their young. No fraudulent paper is required. Positive citation bias works by dropping one hedge at a time until the last image bears almost no resemblance to the first."
  tier="real_science"
/>
...
<DiagramFigure
  ...
  labels={[
    { x: 25, y: 68, side: "right", label: "Idealized Monolithic Grid", note: "Popular metaphor imagining a single seamless, fully connected planetary nervous system." },
    { x: 50, y: 50, side: "right", label: "Metaphorical Boundary", note: "The divide between theoretical superorganism models and empirical soil ecology." },
    { x: 76, y: 56, side: "left", label: "Competing Fungal Genets", note: "Real ecosystems consist of genetically distinct, competing fungal strains with localized resource boundaries." },
    { x: 82, y: 82, side: "left", label: "Structural Gaps & Physical Barriers", note: "Rocks, soil turnover, and incompatible mycelia produce frequent discontinuities and network partitions." }
  ]}
  alt="The same forest floor shown twice in cutaway: an idealized side where every tree root joins one seamless evenly glowing fungal web, and a wild side where short local fungal patches overlap, end at dark gaps, break, and regrow without forming one continuous organism"
  figNo="08"
  caption="The web people imagine (left) and the ground researchers actually have to map (right). Wild mycorrhizal networks are not permanent municipal cable: fungal individuals occupy local patches, links snap and regrow, and overlapping threads need not belong to one continuous body. The forest remains richly connected. It is simply connected by a shifting mosaic, not by one immortal internet under the soil."
  tier="real_science"
/>
