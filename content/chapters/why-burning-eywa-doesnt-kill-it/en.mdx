<DiagramFigure
  labels={[
    { x: 20, y: 45, side: "right", label: "Intact bioluminescent forest", note: "Densely interconnected canopy and root web holding the planetary network" },
    { x: 50, y: 60, side: "right", label: "Re-knitting lattice", note: "Cyan filaments bridge signals across and around the scarred boundary" },
    { x: 82, y: 50, side: "left", label: "Scorched clear-cut", note: "Physical trees destroyed by fire, but underlying topology reroutes" }
  ]}
...
<DiagramFigure
  ...
  labels={[
    { x: 59, y: 13, side: "left", label: "The aimed strike", note: "Tree of Souls — a true hub, ringed for targeting" },
    { x: 18, y: 80, side: "right", label: "Bulldozed grove", note: "Tree of Voices — a leaf node the network barely misses" },
    { x: 47, y: 53, side: "right", label: "A felled giant", note: "Hometree — massive physical biomass, yet topologically redundant" },
    { x: 82, y: 62, side: "left", label: "Hunted in the deep", note: "Tulkun — mobile nodes removed region by region" }
  ]}
  figNo="01"
...
<DiagramFigure
  ...
  labels={[
    { x: 25, y: 42, side: "right", label: "Hub-connected core", note: "Central high-degree hubs survive random deletions unscathed" },
    { x: 25, y: 84, side: "right", label: "Giant component holds", note: "Random loss hits abundant leaves; global connectivity remains intact" },
    { x: 72, y: 22, side: "left", label: "Targeted attack", note: "Strategic removal aimed selectively at high-degree hubs" },
    { x: 72, y: 52, side: "left", label: "Shattered into islands", note: "Dead hubs (magenta X) sever connections, causing topological collapse" }
  ]}
  figNo="02"
...
<DiagramFigure
  ...
  labels={[
    { x: 19, y: 31, side: "right", label: "The local module", note: "One clan's forest — densely wired within itself" },
    { x: 40, y: 31, side: "left", label: "The articulation point", note: "The single bridge node (pink reticle) — cut it and the graph splits" },
    { x: 74, y: 28, side: "left", label: "The planetary network", note: "The far side closes over the gap and survives intact" },
    { x: 19, y: 79, side: "right", label: "Severed, not killed", note: "Below: the bridge cut — the module goes dark, amputated from the whole" }
  ]}
  figNo="03"
...
<DiagramFigure
  ...
  labels={[
    { x: 18, y: 25, side: "right", label: "Untouched hub forest", note: "Intact sacred tree acts as the broadcast reservoir for seeds" },
    { x: 44, y: 38, side: "left", label: "The drifting seed bank", note: "Atokirina' carried on wind currents across the moon" },
    { x: 67, y: 76, side: "left", label: "Reseeding the gap", note: "Settled woodsprites extend new cyan hyphal threads into sterile ash" }
  ]}
  figNo="04"
...
<DiagramFigure
  ...
  labels={[
    { x: 25, y: 38, side: "right", label: "Resistance (armour)", note: "Thick bark deflects flame; the standing trunk structure remains unchanged" },
    { x: 74, y: 45, side: "left", label: "Resilience (renewal)", note: "Canopy burns to stumps, but underlying function is actively rebuilt" },
    { x: 65, y: 68, side: "left", label: "Recovery reserve", note: "Buried root crowns, fungal threads, and fresh green shoots rapidly resprout" }
  ]}
...
<RegenerationRace
  caption="The race the whole chapter turns on. While reseeding keeps ahead, the standing network settles at a permanent scar and stays connected indefinitely — no amount of further burning at that rate ever reaches the threshold. Push the burn rate up and the two curves cross: repair slows as the forest that powers it shrinks, the slide accelerates, and the giant component crosses the critical density in a single step. This is why Eywa's immortality is real and conditional at once. It is not armour. It is a rate, and a rate can be outrun."
/>
...
<DiagramFigure
  ...
  labels={[
    { x: 16, y: 35, side: "right", label: "Targeted terrestrial hubs", note: "Rare sacred trees circled in magenta across the terrestrial canopy" },
    { x: 39, y: 48, side: "right", label: "Simultaneous coordinated strike", note: "Pinpointing key connector nodes rather than clear-cutting ordinary forest" },
    { x: 69, y: 79, side: "left", label: "Subsea Spirit Tree hubs", note: "Marine neural conduits targeted to decouple oceanic and terrestrial webs" }
  ]}
...
<DiagramFigure
  ...
  labels={[
    { x: 25, y: 65, side: "right", label: "Living recovery reserve", note: "Seeds, mycorrhizal hyphae, woodsprites, and germinants ready to replace canopy" },
    { x: 75, y: 22, side: "left", label: "Deceptive green canopy", note: "Adult trees stand unchanged today, masking subterranean sterility" },
    { x: 75, y: 65, side: "left", label: "Dead subsoil (countdown)", note: "Poisoned soil with dead hyphae and empty seed husks — zero replacement capacity" }
  ]}
...
<DiagramFigure
  ...
  labels={[
    { x: 16, y: 48, side: "right", label: "Intact mutualistic web", note: "Pollinators, seed-dispersing grazers, and vibrant bioluminescent flora" },
    { x: 50, y: 55, side: "right", label: "Fauna removed (silent forest)", note: "Trees stand but unpollinated flowers and rotting seeds arrest recruitment" },
    { x: 83, y: 55, side: "left", label: "Co-extinction cascade", note: "Canopy collapses from lack of regeneration; remaining fauna starve" }
  ]}
