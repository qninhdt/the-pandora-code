...
<DiagramFigure
  ...
  labels={[
    { x: 40, y: 24, side: "right", label: "Century-scale hollow cavities", note: "Large structural cavities require centuries of fungal decay and animal excavation to mature." },
...
If the species is a widespread climax <GlossaryTerm slug="dominant-species">dominant</GlossaryTerm> across the continent, then each felling is a severe *local* habitat collapse — devastating for that community, invisible at the scale of the moon. If, instead, trees that reach Hometree size and develop the hollow core are rare specimens needing unusual conditions, then RDA mining is a systematic driver of something much worse: the functional extinction of a habitat type. The films give us no distribution, no abundance, no count.
