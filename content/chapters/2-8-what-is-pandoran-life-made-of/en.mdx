...
<SolventWindowBench
  caption="Where 'close to a universal optimum' earns its hedge. Sweep a world's surface temperature and watch how few of the four candidates are liquid at any one setting — and how the ones still flowing in the deep cold are precisely the ones no protein can fold in. Then read the columns: formamide beats water on liquid window and on polarity, and only water has ever been shown to dissolve life's molecules, drive protein folding, and let a membrane assemble unaided. Water wins the requirements it must win, and loses two headline numbers to a rival — which is exactly why the verdict is 'close to' optimal rather than optimal."
/>
