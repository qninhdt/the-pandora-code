...
// Render full-bleed landing background images from committed prompts. Mirrors
...
// Output: apps/web/public/images/landing/{id}.png.

const ROOT = process.cwd();
const MANIFEST = path.join(ROOT, "content", "art-direction", "landing-backgrounds.yaml");
const OUT_DIR = path.join(ROOT, "apps/web/public/images/landing");
...
function fail(message: string): never {
  console.error(`\n[gen-landing-backgrounds] ${message}\n`);
  process.exit(1);
}
...
function collectJobs(filterId?: string): Background[] {
  ...
  if (!parsed.success) {
    const issues = parsed.error.issues
      ...
      .join("\n");
    fail(`Invalid landing-backgrounds.yaml:\n${issues}`);
  }
...
async function generateOne(job: Background, styleBible: string, force: boolean): Promise<void> {
  ...
  if (!force && existsSync(outPath)) {
    console.log(`[gen-landing-backgrounds] skip ${job.id} (image exists; use --force)`);
    return;
  }
  ...
  console.log(`[gen-landing-backgrounds] generating ${job.id} (${job.aspect})...`);
  ...
  console.log(`[gen-landing-backgrounds] wrote ${path.relative(ROOT, outPath)}`);
}
...
async function main(): Promise<void> {
  ...
  if (jobs.length === 0) {
    console.log(
      args.id
        ? `[gen-landing-backgrounds] background "${args.id}" was not found.`
        : "[gen-landing-backgrounds] no backgrounds found.",
    );
    return;
  }
  ...
  console.log(
    `[gen-landing-backgrounds] processing ${jobs.length} background(s) with concurrency ${args.concurrency}...`,
  );
  await generateMany(jobs, styleBible, args.force, args.concurrency);
  console.log(`[gen-landing-backgrounds] done. ${jobs.length} background(s) processed.`);
}
